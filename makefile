...
OTHERFLAGS := \
	...
	# -e TZ=Asia/Kolkata \
	# \
	# -e ADMIN_USER_PASS="insecurebydefault" \
	# -e ADMIN_USER_ACCESS_LEVEL="-2" \
	# \
	# -e AUTO_CREATE_USER="feeduser" \
	# -e AUTO_CREATE_USER_PASS="insecurebydefaultuser" \
	# -e AUTO_CREATE_USER_ENABLE_API="true" \
	# -e AUTO_CREATE_USER_ACCESS_LEVEL="0" \
	# \
	# -e NO_STARTUP_SCHEMA_UPDATES=1 \
	# -e NO_STARTUP_PLUGIN_UPDATES=1 \
	# -e S6_NEEDED_PACKAGES="git" \
	# -e TTRSSUPDATE=1 \
	# \
	# -e TTRSS_DB_TYPE=mysql \
	# -e TTRSS_DB_HOST=mysql \
	# -e TTRSS_DB_HOST=mysql --link docker_mysql \
	# -e TTRSS_DB_PORT=3306 \
	# -e TTRSS_DB_USER=mysql \
	# -e TTRSS_DB_PASS=insecurebydefault \
	# -e TTRSS_DB_NAME=ttrss \
	# -e TTRSS_MYSQL_CHARSET="UTF8" \
	# \
	# -e TTRSS_DB_TYPE=pgsql \
	# -e TTRSS_DB_HOST=postgresql \
	# -e TTRSS_DB_HOST=postgresql --link docker_postgresql \
